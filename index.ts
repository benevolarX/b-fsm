export interface IPrepareMachineOptions<
...
//
export interface IPrepareTransitionOptions<
  EventsName extends string,
  StatesName extends string
> {
  by: EventsName;
...
//
export interface IPreparedTransition<
  EventsName extends string,
  StatesName extends string
> {
  by: EventsName;
...
//
export interface IPreparedState<
  EventsName extends string,
  StatesName extends string
> {
...
class State<EventsName extends string, StatesName extends string> {
  ...
  constructor(public id: StatesName) {
    this.transitions = {} as Record<
      EventsName,
      Transition<EventsName, StatesName>
    >;
  }
...
/////////////////////////
function privatePrepareTransition<
  EventsName extends string,
  StatesName extends string
>(
...
function privatePrepareMachine<
  EventsName extends string,
  StatesName extends string
>(opt: IPrepareMachineOptions<EventsName, StatesName | (string & {})>) {
  type StatesNameID = StatesName | typeof opt.id;
  const machine = new Machine<EventsName, StatesNameID, typeof opt.id>(
    opt.id,
    opt.init
  );
  for (const [id, s] of Object.entries<
    IPreparedState<EventsName, StatesNameID>
  >(opt.states)) {
